...
public class FXMLDocumentController implements Initializable {
	...
	private void addTask(ActionEvent event) throws SQLException {
		...
		if (TaskText.getText().isEmpty() || OwnerText.getText().isEmpty()) {
			...
			Data nd = new Data(datePicker.getValue(), OwnerText.getText(), TaskText.getText());
	...
	@FXML
	private void handleButtonSelDate(ActionEvent event) {
		popDownSelDate();
	}
	...
	private void popUpSelDate() {
		basePane.setDisable(true);
		basePane.setOpacity(0.4);
		selDatePane.setVisible(true);

	}

	private String popDownSelDate() {
		basePane.setDisable(false);
		basePane.setOpacity(1);
		selDatePane.setVisible(false);
	...
	public void loadDataFromDatabase() {
		...
		tc3.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Data, String>>() {
			@Override
		...
		tc4.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<Data, String>>() {
			@Override
			public void handle(TableColumn.CellEditEvent<Data, String> t) {
				Data actualdata = t.getTableView().getItems().get(t.getTablePosition().getRow());
		...
		tc1.setCellValueFactory(new Callback<CellDataFeatures<Data, Integer>, ObservableValue<Integer>>() {
			@Override
			public ObservableValue<Integer> call(CellDataFeatures<Data, Integer> p) {
